...
%}
model(2,3) = {@(params)sprintf(['%f + (%f * y) + (%f * xy) + (%f * x)\n' ...
    ' + (%f * y^2) + (%f * (xy)^2) + (%f * x^2)'],params(1),params(2),params(3),params(4),params(5),params(6),params(7))};
model(2,1) = {@(params,in_data)params(1) + params(2)*in_data(2,:)'*ones(1,size(in_data,2)) + params(3)*in_data(2,:)'*in_data(1,:) + params(4)*ones(size(in_data,2),1)*in_data(1,:) ...
...
%}
model(3,3) = {@(params)sprintf(['%f + (%f * y) + (%f * xy) + (%f * x)\n' ...
...
model(3,1) = {@(params,in_data)params(1) + params(2)*in_data(2,:)'*ones(1,size(in_data,2)) + params(3)*in_data(2,:)'*in_data(1,:) + params(4)*ones(size(in_data,2),1)*in_data(1,:) ...
    + params(5)*in_data(2,:).^2'*ones(1,size(in_data,2)) + params(6)*in_data(2,:)'.^2 * in_data(1,:).^2 + params(7)*ones(size(in_data,2),1)*in_data(1,:).^2 ...
...
%}
model(4,3) = {@(params)sprintf(['%f + (%f * y) + (%f * xy) + (%f * x)\n' ...
    ' + (%f * y^2) + (%f * (xy)^2) + (%f * x^2)\n + (%f * y^3) + (%f * (xy)^3) + (%f * x^3)\n' ...
...
model(4,1) = {@(params,in_data)params(1) + params(2)*in_data(2,:)'*ones(1,size(in_data,2)) + params(3)*in_data(2,:)'*in_data(1,:) + params(4)*ones(size(in_data,2),1)*in_data(1,:) ...
    + params(5)*in_data(2,:).^2'*ones(1,size(in_data,2)) + params(6)*in_data(2,:)'.^2 * in_data(1,:).^2 + params(7)*ones(size(in_data,2),1)*in_data(1,:).^2 ...
    + params(8)*in_data(2,:).^3'*ones(1,size(in_data,2)) + params(9)*in_data(2,:)'.^3 * in_data(1,:).^3 + params(10)*ones(size(in_data,2),1)*in_data(1,:).^3 ...
...
%}
model(5,3) = {@(params)sprintf('%f + (%f * y^2) + (%f * xy) + (%f * x^2)',params(1),params(2),params(3),params(4))};
model(5,1) = {@(params,in_data)params(1) + params(2)*(in_data(2,:).^2)'*ones(1,size(in_data,2)) + params(3)*in_data(2,:)'*in_data(1,:) + params(4)*ones(size(in_data,2),1)*in_data(1,:).^2}; 
%{
 model 6: a1 + a2*y^3 + a3*y^2*x + a4*y*x^2 + a5*x^3
%}
model(6,3) = {@(params)sprintf('%f + (%f * y^3) + (%f * x * y^2) + (%f * x^2 * y) + (%f * x^3)',params(1),params(2),params(3),params(4),params(5))};
model(6,1) = {@(params,in_data)params(1) + params(2)*(in_data(2,:).^3)'*ones(1,size(in_data,2)) + params(3)*(in_data(2,:).^2)'*in_data(1,:) + params(4)*in_data(2,:)'*in_data(1,:).^2 + params(5)*ones(size(in_data,2),1)*in_data(1,:).^3}; 

%{
 model 7: a1 + a2*y^4 + a3*y^3*x + a4*y^2*x^2 + a5*y*x^3 + a6*x^4
%}
model(7,3) = {@(params)sprintf('%f + (%f * y^4) + (%f * x * y^3) + (%f * x^2 * y^2) + (%f * x^3 * y) + (%f * x^4)',params(1),params(2),params(3),params(4),params(5),params(6))};
model(7,1) = {@(params,in_data)params(1) + params(2)*(in_data(2,:).^4)'*ones(1,size(in_data,2)) + params(3)*(in_data(2,:).^3)'*in_data(1,:) + params(4)*(in_data(2,:).^2)'*in_data(1,:).^2 + params(5)*(in_data(2,:))'*in_data(1,:).^3 + params(6)*ones(size(in_data,2),1)*in_data(1,:).^4}; 
